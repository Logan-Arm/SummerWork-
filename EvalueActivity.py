...
class Trial():
    ...
    def train_model(self,j):
        ...
        for epoch in range(self.epochs):
            if epoch==0: #appends the first matrix
                ...
                #Saving the current 2nd and 3rd eigenvalues for purposes of activation rates
            ...
            if epoch%self.alignmentint==0 and epoch!=0: #Only include NTK calculations every alignmentint epochs
                ...
                #Seeing whether or not the second and third eigenvalues are still active
                #If absolute relative rate of change of the eigenvalue is greater than 0.2
                # if np.abs(selected_evals[1]- self.eval2_prev)/self.eval2_prev > 0.2:
                #     print(f"Eigenvalue 2 is active at training time{epoch*self.lr}")
                #     self.eval2_activity_j.append(epoch*self.lr)

                # if np.abs(selected_evals[2]- self.eval3_prev)/self.eval3_prev > 0.2:
                #     print(f"Eigenvalue 3 is active at training time{epoch*self.lr}")
                #     self.eval3_activity_j.append(epoch*self.lr)


                #Derivative 
                self.eval1_deriv[j,epoch//self.alignmentint] = np.abs(selected_evals[0] - self.eval1_prev)/(self.alignmentint*self.lr)
                self.eval2_deriv[j,epoch//self.alignmentint] = np.abs(selected_evals[1] - self.eval2_prev)/(self.alignmentint*self.lr)
                self.eval3_deriv[j,epoch//self.alignmentint] = np.abs(selected_evals[2] - self.eval3_prev)/(self.alignmentint*self.lr)

                #Absolute relative rate of change
                self.eval1_rateofchange[j,epoch//self.alignmentint] = np.abs(selected_evals[0] - self.eval1_prev)/(self.eval1_prev)
                self.eval2_rateofchange[j,epoch//self.alignmentint] = np.abs(selected_evals[1] - self.eval2_prev)/(self.eval2_prev)
                self.eval3_rateofchange[j,epoch//self.alignmentint] = np.abs(selected_evals[2] - self.eval3_prev)/(self.eval3_prev)
    ...
    def compute_data(self):
        ...
        """Calculating the derivatives of the eigenvalues"""
