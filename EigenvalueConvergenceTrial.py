...
class Trial():
    def __init__(self, input,output,width,depth,lr,epochs,STD,ensemblenum,performances,bootstraps,alignmentint,X_train,Y_train,x_test,y_test,filename, SaveFig):
        ...
        self.eval_2_time = np.zeros((self.ensemble)) #Stores the training times for the second eigenvalue to hit target value
        self.eval_3_time = np.zeros((self.ensemble)) #Stores the training times for the third eigenvalue to hit target value
    ...
    def train_model(self,j):
        ...
        self.eval_2_convergence = 0
        self.eval_3_convergence = 0
        ...
        for epoch in range(self.epochs):
            if epoch==0: #appends the first matrix
                ...
                evals, evecs = np.linalg.eigh(ntk_matrix) #calculates eigenvectors and eigenvalues
                selected_indices = np.argsort(evals)[-5:][::-1] #Takes last 2 values of the sorted eigenvalues
                #[::-1] puts in descending order
                selected_evecs = evecs[:, selected_indices]
                selected_evals = evals[selected_indices]
                #Want to test the model's behavior in the time it takes eigenvalues to reach the initial highest eigenvalue--> ergo need to save initial highest eigenvalue
                self.eval_target = selected_evals[0] #Grabs the first eigenvalue from the list
                for z in range(2):
            ...
            if epoch%100==0 and epoch!=0: #Only include NTK calculations every 100 epochs
                ...
                selected_evecs = evecs[:,selected_indices]
                selected_evals = evals[selected_indices]
                if selected_evals[1] >= self.eval_target and self.eval_2_convergence ==0:
                    self.eval_2_time[j] = epoch*self.lr
                    self.eval_2_convergence =1 
                    print(f"Second eigenvalue has hit target value at training time {epoch *self.lr}")
                if selected_evals[2] >= self.eval_target and self.eval_3_convergence ==0:
                    self.eval_3_time[j] = epoch*self.lr
                    self.eval_3_convergence =1
                    print(f"Third eigenvalue has hit target value at training time {epoch *self.lr}")
    ...
    def compute_data(self):
        ...
        """Calculating the times of interest for the second and third eigenvalue convergence"""
        self.eval_2_time_mean = np.mean(self.eval_2_time, axis = 0)
        self.eval_2_time_uncert = np.std(self.eval_2_time, axis =0)/np.sqrt(self.ensemble)

        self.eval_3_time_mean = np.mean(self.eval_3_time, axis = 0)
        self.eval_3_time_uncert = np.std(self.eval_3_time, axis =0)/np.sqrt(self.ensemble)
    ...
    def make_plots(self):
        ...
        for k in range(len(self.ensemble_means)): 
            ...
            ax.plot(self.train_time_rate, mean, label=f'Layer {k+1}') #Convention is to use layer 0 as input, so need to shift everything up by 1
            #ax.fill_between(epochs_axis, mean - std, mean + std, alpha=0.3)
        #Adding 2nd eigenvalue points
        ax.axvspan(self.eval_2_time_mean- self.eval_2_time_uncert, self.eval_2_time_mean + self.eval_2_time_uncert, color='red', label = "2nd Eval", alpha=0.3)
        #Axvspan expects a single point, cannot use an array, hence need the for loop
        #Adding third eigenvalue points
        ax.axvspan(self.eval_3_time_mean- self.eval_3_time_uncert, self.eval_3_time_mean + self.eval_3_time_uncert, color='green', label = "3nd Eval", alpha=0.3)    
        ax.set_xlabel('Training Time')
        ax.set_ylabel('Pre-Activation Derivative')
        ax.set_title('Finite Difference of Layer Pre-Activations')
        ax.legend()
        plt.tight_layout()
        if self.SaveFig:
            plt.savefig(fr'C:\Users\Logan\Downloads\SummerWork\{self.Filename}\ActivityEvalTime')
        plt.show()
        plt.close()
        ...
        plt.tight_layout()
        if self.SaveFig:
            plt.savefig(fr'C:\Users\Logan\Downloads\SummerWork\{self.Filename}\ActivityNoOuterEvalTime')
        plt.show()
        plt.close()
        ...
        """Plotting Losses With Eigenvalue Time Metrics"""
        ensemble_loss = np.mean(self.loss_array, axis=1)
        ensemble_loss_uncert = np.std(self.loss_array, axis = 1)/np.sqrt(self.ensemble)
        plt.plot(self.train_time_total,ensemble_loss)
        plt.fill_between(self.train_time_total,ensemble_loss+ensemble_loss_uncert,ensemble_loss-ensemble_loss_uncert, alpha = 0.3)
        plt.axvspan(self.eval_2_time_mean- self.eval_2_time_uncert, self.eval_2_time_mean + self.eval_2_time_uncert, color='red', label = "2nd Eval", alpha=0.3)
        #Axvspan expects a single point, cannot use an array, hence need the for loop
        #Adding third eigenvalue points
        plt.axvspan(self.eval_3_time_mean- self.eval_3_time_uncert, self.eval_3_time_mean + self.eval_3_time_uncert, color='green', label = "3nd Eval", alpha=0.3)   
        plt.xlabel(f'Training time')
        plt.ylabel(f"Average ensemble loss value")
        plt.title(f'Ensemble loss vs training time')
        if self.SaveFig:
            plt.savefig(fr'C:\Users\Logan\Downloads\SummerWork\{self.Filename}\LossEvalueTime')
        plt.show()
        plt.close()
